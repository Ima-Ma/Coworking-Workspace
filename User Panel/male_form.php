...
if (isset($_POST['submit'])) {
    ...
    // File upload paths
    $pdf_dir = 'male_driver/pdf/';
    $image_dir = 'male_driver/image/';
    ...
    if (empty($name) || empty($email) || empty($age) || empty($_FILES['pdf']['name']) || empty($_FILES['image']['name'])) {
        ...
        if (move_uploaded_file($_FILES['pdf']['tmp_name'], $pdf_file) && move_uploaded_file($_FILES['image']['tmp_name'], $image_file)) {
            ...
            $sql = "INSERT INTO male (name, email, pdf, image, age) VALUES ('$name', '$email', '$pdf_file', '$image_file', '$age')";
...
?>
    ...
    <div class="container">
        <h3 class="text-center fw-bold mt-5">Are you a dedicated and reliable Male driver seeking a rewarding role? We’re excited to invite you to apply for our driver positions!</h3>
